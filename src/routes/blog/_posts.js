...
// Use this to separate a post's content from a post's front-matter
...
// Posts are an array of post objects whose properties are important
// elements of the post like post title, date posted, content, etc.
const posts = fs.readdirSync(POSTS_DIR)
  // filter down to only markdown posts
  ...
  .map(fileName => {
    ...
    // Use the file name as a routing slug
    const slug = fileName.split('.')[0]
    let content = rawContent
    let excerpt = ''

    // Search the content for an excerpt separator
    if (rawContent.indexOf(EXCERPT_SEPARATOR) !== -1) {
      const splittedContent = rawContent.split(EXCERPT_SEPARATOR)
      // Pull out the post's preview
      excerpt = splittedContent[0]
      // Pull out the post's content
      content = splittedContent[1]
    }

    // Generate markup for the parsed markdown content
    const html = marked(content)
    // Estimate the article's reading duration
